...
const ProductDetailOverlay = forwardRef((props, formEditProductOverlay) => {
    ...
    const imageProductInput = useRef(null);
    const previewImageProduct = useRef(null);
    const labelImageProductInput = useRef(null);
        ...
        imageProductInput.current.addEventListener('change', function(event) {
            ...
            if (file) {
              ...
              reader.onload = function(e) {
                labelImageProductInput.current.innerHTML = 'Ubah Gambar'
                previewImageProduct.current.src = e.target.result;
                previewImageProduct.current.style.display = 'block';
              }
              reader.readAsDataURL(file);
            } else {
              previewImageProduct.current.style.display = 'none';
            }
          });

          function restrictToNumbers(e) {
            const charCode = e.key;
            if (/[0-9]/.test(charCode) || charCode === 'Backspace' || charCode === 'Delete' || charCode === 'ArrowLeft' || charCode === 'ArrowRight' || charCode === 'Tab' || charCode === 'Enter') {
                  return;
              } else{
                e.preventDefault();
              }
          ...
          function validateNumericInput(inputElement) {
            ...
            inputHarga.current.addEventListener('keydown', restrictToNumbers)
            inputStock.current.addEventListener('keydown', restrictToNumbers)
  ...
  return (
    <>
        <div ref={formEditProductOverlay} className='fixed top-0 left-0 h-screen w-screen bg-[rgba(0,0,0,.5)] hidden justify-center pt-32 z-50'>
            <div className='bg-white border border-gray-200 w-80 h-fit px-8 pb-8 pt-4 rounded-lg flex flex-col'>
                <button className='self-end cursor-pointer translate-x-4' type='button' onClick={() => {formEditProductOverlay.current.classList.replace('flex', 'hidden');formEditProduct.current.reset();}}>
                ...
                <h2 className='mt-2 text-center text-2xl font-extrabold text-slate-900 select-none mb-6'>Detail Produk</h2>
                <div ref={alertForm} className='justify-center bg-red-300 text-white py-1 px-2 rounded-sm hidden'>
                    <p ref={textAlertForm}>Alert!</p>
                </div>
                <form ref={formEditProduct} action={import.meta.env.VITE_API_URL+'/product/update'} method="post" className='flex flex-col gap-2 mt-2'>
                    <input type="text" name="id" hidden value={props.product.idProduct} />
                    <input ref={inputNama} name='name' className='border border-gray-200 rounded-sm p-1' type="text" placeholder={props.product.nameProduct} required />
                    <input ref={inputHarga} name='price' className='border border-gray-200 rounded-sm p-1' type="number" placeholder={props.product.priceProduct} required />
                    <input ref={inputStock} name='stock' className='border border-gray-200 rounded-sm p-1' type="number" placeholder={props.product.stockProduct} required />
                    <div className='flex flex-col gap-2'>
                         <label htmlFor='input-gambar' className="hidden border border-gray-200 p-2 rounded-sm w-fit cursor-pointer hover:bg-gray-200 gap-1 items-center">
                            ...
                            <span ref={labelImageProductInput}>Pilih Gambar</span>
                        </label>
                         <input ref={imageProductInput} id="input-gambar" name="image" type="file" accept=".jpg, .jpeg, .png, .webp" className="border border-gray-300 p-2 text-center cursor-pointer hover:bg-gray-200 rounded-lg" required/>
